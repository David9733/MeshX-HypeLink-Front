...
| 역할 | 주요 행동 흐름 |
|------|--------------|
| **본사 (ADMIN)** | 전체 매장 현황 확인 → 발주 승인 → POS 상태 점검 → 고객 분석 → 프로모션 생성 → 공지 발송 |
| **서브관리자 (MANAGER)** | 재고·배송 현황 조회 → 배송 기사 배정 → 매출 분석 → AS 처리 |
| **가맹점 (BRANCH_MANAGER)** | 내 매장 재고 확인 → 발주 요청 → AS 신청 → 프로모션 조회 |
