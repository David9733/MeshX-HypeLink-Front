...
— **템플릿 선정** : 프론트엔드 개발 초기 메인 페이지에 적합한 UI 템플릿을 찾는 데 어려움을 겪었습니다. 찾는 과정에서 GitHub에도 다양한 템플릿이 공개되어 있다는 것을 알게 됐고, 다른 나라의 템플릿도 생각보다 완성도가 높아 메인 페이지 구현에 실질적인 도움이 됐습니다.

— **POS 화면 설계** : POS 화면 설계 초기에 진행 방향을 잡는 데 어려움이 있었습니다. Toss POS를 벤치마킹해 따라갈 부분과 수정할 부분을 정리하고, 바이브 코딩을 활용해 빠르게 구현해나갔습니다. 메인 페이지도 같은 방식으로 바이브 코딩을 통해 수정해 나가면서, 방향성이 명확하면 구현 속도를 크게 높일 수 있다는 것을 체감했습니다.
