...
| 역할별 접근 설계 | `hypelinkMain/src/stores/permissions.js`<br>`hypelinkMain/src/router/index.js` | 본사·서브관리자·가맹점 역할별로 사이드바<br>노출 메뉴 항목과 라우터 허용 경로를 설계<br>사이드바 숨김만으로는 URL 직접 접근을 차단할 수 없어 라우터 가드에서 역할 검증을 추가 |
...
| POS설계 | `hypelinkPos/src/views/POSView.vue` | POS 벤치마킹 기반 UI/UX 설계<br>시나리오 점검을 통한 오류 이슈화 품질 개선 |
