...
| 역할별 접근 제어 설계 | `hypelinkMain/src/stores/permissions.js`<br>`hypelinkMain/src/router/index.js` | 본사·서브관리자·가맹점 역할별로 사이드바 노출 메뉴 항목과 라우터 허용 경로를 별도 설계<br>사이드바 숨김만으로는 URL 직접 접근을 차단할 수 없어 라우터 가드에서 역할 검증을 추가 |
