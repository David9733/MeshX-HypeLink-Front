...
이전 프로젝트에서 ERD를 수작업으로 그리다 보니 설계 단계에 시간이 과도하게 소요됐습니다. IntelliJ에서 Java CRUD 코드를 생성하면 ERD가 자동으로 만들어지는 기능을 발견하여 적용했고, 설계 시간을 크게 단축할 수 있었습니다. 이 경험을 통해 개발 도구의 자동화 기능을 먼저 파악하는 것만으로도 반복 작업을 줄이고 개발 본질에 집중할 수 있다는 것을 배웠습니다.

### Frontend — 템플릿 선정

프론트엔드 개발 초기 메인 페이지에 적합한 UI 템플릿을 찾는 데 어려움을 겪었습니다. 찾는 과정에서 GitHub에도 다양한 템플릿이 공개되어 있다는 것을 알게 됐고, 다른 나라의 템플릿도 생각보다 완성도가 높아 메인 페이지 구현에 실질적인 도움이 됐습니다.

### Frontend — POS 화면 설계

POS 화면 설계 초기에 진행 방향을 잡는 데 어려움이 있었습니다. Toss POS를 벤치마킹해 따라갈 부분과 수정할 부분을 정리하고, 바이브 코딩을 활용해 빠르게 구현해나갔습니다. 메인 페이지도 같은 방식으로 바이브 코딩을 통해 수정해 나가면서, 방향성이 명확하면 구현 속도를 크게 높일 수 있다는 것을 체감했습니다.

### 협업 — 이슈 트래킹 도구 선택

팀 이슈 및 일정 관리를 위해 Jira 도입을 검토했으나, 초기 설정 복잡도와 팀 학습 비용이 높아 실제 개발에 집중하기 어려운 상황이었습니다. Jira는 대규모 팀·장기 프로젝트에 최적화된 도구로 단기 프로젝트에서는 오버엔지니어링이 될 수 있다고 판단해, GitHub Actions와 PR 기반 워크플로우로 대체했습니다. 별도 도구 없이 GitHub 단일 환경에서 이슈 트래킹·CI·코드 리뷰를 일원화하면서 팀 규모와 일정에 맞는 도구 선택이 협업 효율에 직결된다는 것을 배웠습니다.
